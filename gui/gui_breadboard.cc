                                   ...
                                   Breadboard_Window *bbw)
{
  ...
  case GDK_BUTTON_RELEASE:
    if (dragging) {
      ...
      //treeselect_module(dragged_module); //RRR new
    }
...
/*
 * Search through the treeview list for the selected module.
 * select the entry in the treeview list.
 *
 * bbw->modules is a vector of modules which does not include the node entry
 * hence path uses row+1
 */
void Breadboard_Window::select_in_treelist(Breadboard_Window *bbw)
{
    GtkTreePath *path = nullptr;
    unsigned int row;
    for(row=0; row< bbw->modules.size(); row++)
    {
        if (bbw->modules[row] == bbw->selected_module)
        {
            path = gtk_tree_path_new_from_indices(row+1, -1);
#ifdef DEBUG
            char *path_char = gtk_tree_path_to_string (path);
            fprintf(stderr, "DEBUG select_in_treelist module=%s path=%s\n", bbw->modules[row]->module()->name().c_str(), path_char);
            g_free(path_char);
#endif
            gtk_tree_view_set_cursor ((GtkTreeView*)bbw->tree, path, nullptr, FALSE);
            gtk_tree_path_free(path);
            return;
        }
    }
...
static void trace_all(GtkWidget *, Breadboard_Window *bbw)
{
  ...
  g_object_get(bbw->tree, "model", &tree_store, nullptr);
...
void Breadboard_Window::Build()
{
  ...
  //gtk_paned_set_position(GTK_PANED(hpaned1), 246);
