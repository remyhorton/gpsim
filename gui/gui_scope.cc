                                 ...
                                 Scope_Window *sw)
{
  ...
  switch (key->keyval) {
  case GDK_KEY_plus:
  case GDK_KEY_equal: // usually plus without the shift
    sw->zoomPercent(141);
    break;
  case GDK_KEY_z:
    sw->zoom(2);
...
/* Logical level to wave position */
int Waveform::calcYPosition(bool level) {
  return level ? height / 16 : (height - 3);
}
...
void Waveform::PlotTo(cairo_t *cr, timeMap &left, timeMap &right)
{
  ...
  int nextEvent = calcYPosition(m_logger.get_state(right.eventIndex) == '1');
  ...
  if (nEvents > 1) {
    ...
    if (left.pos != right.pos) {
      cairo_move_to(cr, left.pos, calcYPosition(true) + yoffset);
      cairo_line_to(cr, left.pos, calcYPosition(false) + yoffset);
      cairo_stroke(cr);
    }

    // variations of yellow
    cairo_set_source_rgb(cr, 1.0, 1.0, double(c) / 65535.0);
    cairo_move_to(cr, right.pos, calcYPosition(true) + yoffset);
    cairo_line_to(cr, right.pos, calcYPosition(false) + yoffset);
    cairo_stroke(cr);
    cairo_restore(cr);
...
void Waveform::draw(cairo_t *cr)
{
  ...
  cairo_rectangle(cr, 0.0, yoffset, width, height);
  cairo_fill(cr);
  ...
  left.event = calcYPosition(m_logger.get_state(left.eventIndex) == '1');
  m_last = left;
  right.pos = width;
  right.time = m_stop;
...
void Scope_Window::gridPoints(guint64 *uiStart, guint64 *uiEnd)
{
  ...
  if (dt > 1.0) {
    ...
    double ta = floor(t1 / dt_tic);
    double tb = floor(t2 / dt_tic);

    for (double t = ta; t <= tb; t += 1.0) {
      double ttic = t * dt_tic;
      guint64 uiTime = (guint64)(floor(ttic));
      int pixel = mapTimeToPixel(uiTime);
      if (pixel >= 0) {
        m_MajorTicks.m_pixel[iMajor] = pixel;
        if (nTics >= 5.0 && ((int)t % 2) != 0) 
        {
            uiTime = ~(guint64)0; // Skip every second, text would be too close
        }
      ...
      for (int it = 1; it < nMinorTics; it++, ttic += dt_minor) {
        ...
        int pixel = mapTimeToPixel(uiTime);
        if (pixel >= 0) {
          m_MinorTicks.m_pixel[iMinor] = pixel;
...
bool Scope_Window::selectSignalName(int y)
{
  ...
  int nr = 0;
  for (; i != signals.end(); ++i, ++nr) {
    const int waveHeight = calcWaveHeight(nr);
    const int yoffset = calcWaveYOffset(nr);
    if (y >= yoffset && y < yoffset + waveHeight) {
      if (m_entry->isSelected(*i)) {
        return false;
      }
...
{
  int tickHeight = height / 10; // Scalable tick height
  if (tickHeight < 3) 
  {
    tickHeight = 3;
  }
  ...
  for (int i = 0; i < sw->MajorTicks().sze(); ++i) {
    gdk_cairo_set_source_color(cr, &grid_line_color);
    int x = sw->MajorTicks().pixel(i);
    cairo_move_to(cr, x, height - 2 - tickHeight * 2);
    cairo_line_to(cr, x, height - 2);
    guint64 uiTime = sw->MajorTicks().cycle(i);
    if (uiTime != ~(guint64)0)
    {
      char buff[100];
      g_snprintf(buff, sizeof(buff), "%" PRINTF_GINT64_MODIFIER "d", uiTime);
      pango_layout_set_text(m_TicText, buff, -1);
      int text_height;
      int text_width;
      pango_layout_get_pixel_size(m_TicText, &text_width, &text_height);
      text_width /= 2;
      x = ((x + text_width) > width) ? (width - text_width) : x;
      x = ((x - text_width) < 0) ? 0 : (x - text_width);
      cairo_move_to(cr, x, (height - 2 - text_height) / 2);
  ...
  for (int i = 0; i < sw->MinorTicks().sze(); ++i) {
    ...
    cairo_move_to(cr, x, height - 2 - tickHeight);
    cairo_line_to(cr, x, height - 2);
  }
                                     ...
                                     GdkEventExpose *, Scope_Window *sw)
{
  ...
  cairo_line_to(cr, xpos, sw->height);
  cairo_stroke(cr);
  cairo_destroy(cr);
  return TRUE;
}
...
void Scope_Window::Build()
{
  ...
  gtk_paned_set_position(GTK_PANED(m_pHpaned), 60);
  guint64 start, stop;
  gridPoints(&start, &stop);
  m_TimeAxis->Build(m_PixmapWidth, calcTimeHeight(), 0);
 ...
 * @param   h       New width of the window
 */
void Scope_Window::setSize ( int w, int h )
{
    m_PixmapWidth = w;
    height = h;
    std::vector<Waveform *>::iterator i;

    m_TimeAxis->setSize(w, calcTimeHeight());
    m_TimeAxis->set_yoffset(0);
    int nr = 0;
    for (i = signals.begin(); i != signals.end(); ++i, ++nr) {
      (*i)->set_yoffset(calcWaveYOffset(nr));
      (*i)->setSize(w, calcWaveHeight(nr));
    }
    Update();
}

/* The time scale height is constant for now */
int Scope_Window::calcTimeHeight()
{
  return 30;
}

/* The waveforms can have the remaining space */
int Scope_Window::calcWaveHeight()
{
  return height - calcTimeHeight();
}

/* Height of the wave is between the offsets. Depends on which row it is on. */
int Scope_Window::calcWaveHeight(int nr)
{
  return calcWaveYOffset(nr + 1) - calcWaveYOffset(nr);
}

/* Just divide the space evenly across rows */
int Scope_Window::calcWaveYOffset(int nr)
{
  return calcTimeHeight() + calcWaveHeight() * nr / signals.size();
}
...
{
  zoomPercent(i * 100); // backwards compatibility
}

void Scope_Window::zoomPercent(int i)
{
  m_bFrozen = true;
...
/// mapTimeToPixel - convert time to a pixel horizontal offset. Negative if off screen
int Scope_Window::mapTimeToPixel(guint64 time)
{
  gdouble span = getSpan();
  guint64 start = m_Markers[eStart]->getVal();
  return (int)((time > start && time <= (start + span)) ? ((time - start) * m_PixmapWidth) / span : -1);
}
