...
public:
  ...
  static void toggle_cb(GtkToggleButton *widget,
                        TabButton    *This);
  void set_active();
  GtkWidget * getButton() { return m_button;}
...

  {
    ...
    m_None  = new TabButton(tabVbox,
		gtk_radio_button_new_with_label_from_widget(rb,"none"),
      		-1, this);
    setTabActive(m_currentTabPosition);
...
//------------------------------------------------------------------------
//
// Find the TabButton by it's ID and make it the active radio button
void SourceBrowserPreferences::setTabActive(int id)
{
    GtkWidget *button;

    if (m_Left->get_id() == id)
	button = m_Left->getButton();
    else if (m_Right->get_id() == id)
	button = m_Right->getButton();
    else if (m_Up->get_id() == id)
	button = m_Up->getButton();
    else if (m_Down->get_id() == id)
	button = m_Down->getButton();
    else
    	button = m_None->getButton();
