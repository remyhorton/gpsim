  ...
  NSourcePage *page)
{
  ...
  Dprintf(("SourcePage::KeyPressHandler Received key press 0x%xfor view. line=%d page%p\n", key->keyval, line, page));
  ...
  NSourcePage *pPage)
{
    ...
    GTK_TEXT_WINDOW_LEFT))
  {
    ...
    Dprintf(("NSourcePage::ViewExposeEventHandler x=%d y=%d pPage=%p\n", (int)(pEvent->area.x), y1, pPage));
...
/*
  According to gpasm a label is:
  [.]?[a-z_\x80-\xff?@#][a-z_0-9\x80-\xff.?@#]*
*/
static int isLabel(const char *cP)
{
  int i = 0;
  bool dot = *cP == '.';
  ...
  if (dot)
    cP++;
  if (isalpha(*cP) || *cP == '_' || (*cP & 0x80) != 0 || *cP == '?' || *cP == '@' || *cP == '#')
  {
    while (isalnum(cP[i]) || cP[i] == '_' || (cP[i] & 0x80) != 0 || cP[i] == '.' || cP[i] == '?' || cP[i] == '@' || cP[i] == '#')
      i++;
  }
  return dot ? (i + 1) : i;
}
...
void SourceBuffer::parseLine(const char *cP, int parseStyle)
{
  ...
  while (!isEnd(cP[i])) {
    ...
    if ( (j = isNumber(&cP[i])) != 0) {
      addTagRange("Constants", i + offset, i + j + offset);
      i += j;
      continue;
    }
    if ( (j = isComment(&cP[i])) != 0) {
