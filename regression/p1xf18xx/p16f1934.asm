...
; <http://www.gnu.org/licenses/lgpl-2.1.html>.
        ...
        ;; simulate a pic 16F1934.
        ;; Specifically, basic port operation, eerom, interrupts,
	;; a2d, dac, SR latch, capacitor sense, and enhanced instructions


	list    p=16f1934                ; list directive to define processor
	include <p16f1934.inc>           ; processor specific variable definitions
...
RESET_VECTOR  CODE    0x000              ; processor reset vector
  ...
  .sim "p16f1934.xpos = 72"
  .sim "p16f1934.ypos = 72"
...
;        movwf   status_temp
	...
	.assert "'***FAILED p16f1934 unexpected interrupt'"
	nop
...
;	;
	...
	clrf	STATUS
	clrf	TRISA
   .assert "trisa == 0x00, '*** FAILED 16f1934  TRISA not clear '"
	nop
	BANKSEL PORTA
	movlw	0xff
	movwf	PORTA
   .assert "porta == 0xc0, '*** FAILED 16f1934  analog bits read 0'"
	nop
	movf	PORTA,W
...
;
  ...
  .assert  "'*** PASSED 16f1934 Functionality'"
	nop
	reset
	goto	$
...
test_sr:
    ...
    .assert "(porta&0x30) == 0x20, '*** FAILED p16f1934 RS Q=0'"
	nop
	banksel PORTB
	bsf 	PORTB,0
   .assert "(porta&0x30) == 0x10 , '*** FAILED p16f1934 RS Q=1'"
	nop
	banksel	SRCON1
	movlw	1<<SRRPE
	movwf	SRCON1
    .assert "(porta&0x30) == 0x20, '*** FAILED p16f1934 RS Q=0 SRRPE'"
	nop
	bsf 	SRCON1,SRSPE
    .assert "(porta&0x30) == 0x20 , '*** FAILED p16f1934 RS Q=0 SRRPE & SRSPE'"
	nop
	clrf 	SRCON1
	bsf 	SRCON0,SRPS
    .assert "(porta&0x30) == 0x10, '*** FAILED p16f1934 RS Q=1 SRPS'"
	nop
	bsf 	SRCON0,SRPR
    .assert "(porta&0x30) == 0x20 , '*** FAILED p16f1934 RS Q=0 SRPR'"
	nop
...
test_a2d_ref:
	...
	call	a2dConvert
   .assert "adresh == 0x00 && (adresl==0x96), '*** FAILED 16f1934 AN12=1V V-=Vref-pin'"
	nop
	...
	movwf   ADCON0      	
	call	a2dConvert
   .assert "adresh == 0x01 && (adresl==0x9b), '*** FAILED 16f1934 V+=AN12=1V V-=AN1=2.5V 2's comp'"
	nop
	BANKSEL DACCON0
	clrf	DACCON0
	return
...
test_pir_pie_bits:
	...
	movlw	0xff
	movwf	PIR1
   .assert "pir1 == 0xcf, '*** FAILED 16f1934 PIR1 write test'"
	nop
	clrf	PIR1
	movwf	PIR2
   .assert "pir2 == 0xfd, '*** FAILED 16f1934 PIR2 write test'"
	nop
	clrf	PIR2
	movwf	PIR3
   .assert "pir3 == 0x7a, '*** FAILED 16f1934 PIR3 write test'"
	nop
	clrf	PIR3

	BANKSEL	PIE1
	movwf	PIE1
   .assert "pie1 == 0xff, '*** FAILED 16f1934 PIE1 write test'"
	nop
	clrf	PIE1
	movwf	PIE2
   .assert "pie2 == 0xfd, '*** FAILED 16f1934 PIE2 write test'"
	nop
	clrf	PIE2
	movwf	PIE3
   .assert "pie3 == 0x7a, '*** FAILED 16f1934 PIE3 write test'"
	nop
	clrf	PIE3
	return
test_dac:
...
;RRR	bsf     ANSELC,0	;Set RC0 to analog
	...
	call	a2dConvert
   .assert "(adresh==0x00) && (adresl==0x00), '*** FAILED 16f1934 DAC default'"
	nop
	...
	BANKSEL ADCON0
	call	a2dConvert
   .assert "(adresh==0x02) && (adresl==0x00), '*** FAILED 16f1934 DAC enabled 1/2'"
	nop
	...
	BANKSEL ADCON0
	call	a2dConvert
   .assert "(adresh==0x01) && (adresl==0xa3), '*** FAILED 16f1934 DAC enabled 1/2 FVR '"
	nop
	banksel TRISA
	bsf	TRISA,5
	...
	movwf   ADCON0      	
	call	a2dConvert
   .assert "(adresh==0x01) && (adresl==0xff), '*** FAILED 16f1934 DAC output 1/2 Vdd '"
	nop
...
;RRR	clrf	ANSELC
	...
	movwf   ADCON0      	
	call	a2dConvert
   .assert "adresh == 0x33 && (adresl==0x40), '*** FAILED 16f1934 AN12=1V'"
	nop
  ...
  ; measure FVR (2.048)
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x68) && (adresl==0xc0), '*** FAILED 16f1934 ADC FVR'"
	nop
  ...
  ; measure FVR using FVR Reference
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x03) && (adresl==0xff), '*** FAILED 16f1934 ADC FVR with FVR Ref'"
	nop
...
eefail:
  .assert "'***FAILED 16f1934 eerom write/read error'"
	nop
...
test_int:
	...
	movlw	0x7f
	movwf	INTCON
   .assert "intcon == 0x7e, '*** FAILED 16f1934 INT test - INTCON:IOCIF read only'"
	nop
	BANKSEL	IOCBF 
	movlw	0xff
	movwf	IOCBF
 .assert "iocbf == 0xff, '*** FAILED 16f1934 INT test - IOCBF writable bits'"
	nop
	clrf	IOCBF
	clrf	INTCON
 	...
 	BANKSEL PORTB
        ...
        nop
        movf    inte_cnt,w
   .assert "W == 0x01, '*** FAILED 16f1934 INT test - No int on rising edge'"
        nop
   .assert "iocbf == 0x04, '*** FAILED 16f1934 IOCBF bit 2 not set rising edge'"
	nop
        clrf    inte_cnt
	BANKSEL IOCBF
	clrf	IOCBF
	BANKSEL PORTB
        ...
        nop
        movf    inte_cnt,w
   .assert "W == 0x00, '*** FAILED 16f1934 INT test - Unexpected int on falling edge'"
        nop
...
;	Setup - edge interrupt
	...
	BANKSEL INTCON
        ...
        nop
        movf    inte_cnt,w
   .assert "W == 0x00, '*** FAILED 16f1934 INT test - Unexpected int on rising edge'"
        nop
        clrf    inte_cnt
	bsf	INTCON,IOCIE
        bcf     PORTB,5          ; make a falling edge
        nop
        movf    inte_cnt,w
   .assert "W == 0x01, '*** FAILED 16f1934 INT test - No int on falling edge'"
        nop
   .assert "iocbf == 0x04, '*** FAILED 16f1934 IOCBF bit 2 not set falling edge'"
	nop
...
read_config_data:
   ...
   .assert "eedath == 0x23 && eedatl == 0x40, '*** FAILED 16f1934 Device ID'"
	nop
  ...
  ; test write which is in EEDATAH and EEDATAL to userID1
	...
	btfss	PIR2,EEIF
        goto	$-1
  .assert "UserID2 == 0x2340, '*** FAILED 16f1934 write to UserID2'"
	nop
	return
