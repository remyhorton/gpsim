...
start:
   ...
   .sim "node na0"
   .sim "attach na0 V3.pin porta3 porta2" ; C1IN+, C2IN+
   .sim "node na1"
   ...
   .sim "attach na3 V2.pin porta0"	; C12IN0-
	...
	clrf	ANSELE
	bsf	ANSELE,0	; select AN5
	BANKSEL TRISA
	bcf	TRISA,4		; porta4 output
	bcf	TRISA,5		; porta5 output
  ...
  .assert  "'*** PASSED 16F1939 comparator test'"
	nop
	goto	$-1
...
test_sr:
	...
	bsf 	CM1CON0,C1POL
	; checking SRQ
    .assert "(porta&0x10) == 0x10, '*** FAILED 16f1939 CM1 set SR latch'"
	nop
	bcf 	CM1CON0,C1POL
	; toggle CM2
	bcf 	CM2CON0,C2POL
    .assert "(porta&0x10) == 0x00, '*** FAILED 16f1939 CM2 reset SR latch'"
	nop
	bsf 	CM2CON0,C2POL
	return
;
...
;	bsf 	T1GCON,TMR1GE
	...
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, 'FAILED 16f1939 T1 running CM2 gate off'"
	nop
	...
	nop
	SUBWF	TMR1L,W
  .assert "W != 0, 'FAILED 16f1939 T1 CM2 gate invert'"
	nop
	BANKSEL CM2CON0
	...
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, 'FAILED 16f1939 T1 running gate on'"
	nop
...
test_cm1_Tog:
    ...
    .assert "(pir1 & 0x80) == 0x80, '*** FAILED 16f1939 TMR1GIF set'"
	nop
	bcf 	PIR1,TMR1GIF
	movf	TMR1L,W		; is Timer 1 running ?
	nop
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, 'FAILED 16f1939 T1 gate toggle set, t1 stopped'"
	nop
	bcf 	T1GCON,T1GPOL	; low-high gate transition
        nop
  .assert "(t1gcon & 0x04) != 0, 'FAILED 16f1939 T1 gate toggle set, T1GVAL set'"
	nop
	bsf 	T1GCON,T1GPOL	; high low gate transition
	; T1 should still be running
  .assert "(t1gcon & 0x04) != 0, 'FAILED 16f1939 T1 gate toggle set, T1GVAL set - edge'"
	nop
	bcf 	T1GCON,T1GPOL	; low-high gate transition
	; T1 should be stopped
  .assert "(t1gcon & 0x04) == 0, 'FAILED 16f1939 T1 gate toggle set, T1GVAL clear 2nd +edge'"
	nop
	bsf 	T1GCON,T1GPOL	; high low gate transition
	bcf 	T1GCON,T1GPOL	; low-high gate transition
  .assert "(t1gcon & 0x04) != 0, 'FAILED 16f1939 T1 gate toggle set, T1GVAL set 3rd + edge'"
	nop
...
test_t0_SPM_Tog:
	...
	GOTO	$-1
   .assert "tmr1l == 0x80, '*** FAILED 16f1939 Tmr1 T0 gate toggle'"
	nop
	; wait for next tmr0 overflow, tmr1 should not restart
	bcf 	INTCON,TMR0IF
	btfss	INTCON,TMR0IF
	goto	$-1
   .assert "tmr1l == 0x80, '*** FAILED 16f1939 Tmr1 T0 gate toggle has stopped'"
	nop
  	banksel OPTION_REG
...
test_spm
	...
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, 'FAILED 16f1939 T1 not running armed SPM'"
	nop
  .assert "(t1gcon & 0x04) == 0, 'FAILED 16f1939 T1GVAL clear armed SPM'"
	nop
	...
	nop
	SUBWF	TMR1L,W
  .assert "W != 0, 'FAILED 16f1939 T1 running SPM'"
	nop
  .assert "(t1gcon & 0x04) != 0, 'FAILED 16f1939 T1GVAL set running SPM'"
	nop
	bsf 	T1GCON,T1GPOL	; invert gate control
	movf	TMR1L,W		; is Timer 1 running ?
	nop
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, 'FAILED 16f1939 T1 stopped after SPM'"
	nop
  .assert "(t1gcon & 0x0c) == 0, 'FAILED 16f1939 T1GVAL,T1GGO clear after running SPM'"
	nop
	bcf 	T1GCON,T1GPOL	; invert gate control
	movf	TMR1L,W		; is Timer 1 running ?
	nop
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, 'FAILED 16f1939 T1 not running 2nd pulse SPM'"
	nop
  .assert "(t1gcon & 0x04) == 0, 'FAILED 16f1939 T1GVAL clear 2nd pulse SPM'"
	nop
...
test_compar:
	BANKSEL CM1CON0
  .assert "cm1con0 == 0x04, 'FAILED 16f1939 cm1con0 unexpected init value'"
	nop
  .assert "cm2con0 == 0x04, 'FAILED 16f1939 cm2con0 unexpected init value'"
	nop
  .assert "cm1con1 == 0x00, 'FAILED 16f1939 cm1con1 unexpected init value'"
	nop
  .assert "cm2con1 == 0x00, 'FAILED 16f1939 cm2con1 unexpected init value'"
	nop
	movlw	0xff
	movwf   CM2CON1		; test writable bits
  .assert "cm2con1 == 0xf3, 'FAILED 16f1939 cm2con1 writable bits'"
	nop
	movlw	(1<<C2NCH0)	; CM2 use CN12N1- (portc1), C2N+, no intterupts
	movwf	CM2CON1
  .assert "cmout == 0, 'FAILED 16f1939 cmout default 0'"
	nop
        call 	off_pol
	movlw 	(1<<C1ON)	; Enable Comparator use C1IN+, C12IN0
	movwf	CM1CON0
  .assert "cm1con0  == 0xC0, 'FAILED 16f1939 cm1con0 ON=1 C1OUT=1'"
	nop
	bsf 	CM1CON0,C1POL	; toggle ouput polarity
  .assert "cm1con0  == 0x90, 'FAILED 16f1939 cm1con0 ON=1 POL=1 C1OUT=0'"
	nop
	bsf 	CM1CON0,C1OE	; C1OUT to RA4
   .assert "(porta & 0x10) == 0x00, 'FAILED 16f1939 CM1 C1OUT not low'"
	nop 
	bcf 	CM1CON0,C1POL	; toggle ouput polarity
   .assert "(porta & 0x10) == 0x10, 'FAILED 16f1939 CM1 C1OUT(RA2) not high'"
	nop
	; turn on CM2
	movlw 	(1<<C2ON)|(1<<C2OE)	; Enable Comparator use C2IN+, C12IN0
	movwf	CM2CON0
   .assert "(porta & 0x20) == 0x20, 'FAILED 16f1939 CM1 C2OUT(RC5) high'"
	nop
	BANKSEL CM2CON0
	bsf	CM2CON0,C2POL
   .assert "(porta & 0x20) == 0x00, 'FAILED 16f1939 CM1 C2OUT(RC5) low'"
	nop
	; Test change in voltage detected
   .command "V3.voltage = 2.0"
	nop
   .assert "(porta & 0x10) == 0x00, 'FAILED 16f1939 compare RA2 not low volt change'"
	nop
   .assert "(porta & 0x20) == 0x20, 'FAILED 16f1939 compare RC4 not high volt change'"
	nop
	bsf 	CM1CON1,C1INTP	; interrupt on positive edge
	bsf 	CM1CON0,C1POL   ; change state
   .assert "int_cm1 == 1, 'FAILED 16f1939 cm1 +edge interrupt'"
	nop
	clrf	int_cm1		
	bcf 	CM1CON0,C1POL
   .assert "int_cm1 == 0, 'FAILED 16f1939 cm1 unexpected +edge interrupt'"
	nop
	bsf 	CM2CON1,C2INTN  ; interrupt cm2 on negative edge
	bcf 	CM2CON0,C2POL   ; change state
   .assert "int_cm2 == 1, 'FAILED 16f1939 cm2 -edge interrupt'"
	nop
	clrf	int_cm2
	bsf 	CM2CON0,C2POL   ; change state
   .assert "int_cm2 == 0, 'FAILED 16f1939 cm2 unexpected -edge interrupt'"
	nop
	nop
...
dummy:
	bsf	CM1CON0,C1POL
  .assert "cmout == 1, 'FAILED 16f1939 cmout CM1 off C1POL=1'"
	nop
  .assert "cm1con0 == 0x54, 'FAILED 16f1939 cm1con0 CM1 off C1POL=1'"
	nop
	bsf	CM2CON0,C2POL
  .assert "cmout == 3, 'FAILED 16f1939 cmout CM2 off C2POL=1'"
	nop
  .assert "cm2con0 == 0x54, 'FAILED 16f1939 cm2con0 CM2 off C2POL=1'"
	nop

	bsf 	CM2CON0,C2OE	; C2OUT to RC4
   .assert "(portc & 0x10) == 0x10, 'FAILED 16f1939 CM2 C2OUT not high ON=0, POL=1'"
	nop 
	return
...
interrupt:
	...
	btfsc	INTCON,ADIE
	 btfsc	PIR1,ADIF
	  goto	check
   .assert "'FAILED 16F1939 unexpected interrupt'"
	nop
back_interrupt:
	retfie
