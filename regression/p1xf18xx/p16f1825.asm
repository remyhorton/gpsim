...
ioc_int
	incf	inte_cnt,F
	bcf	INTCON,IOCIE	; stop interrupts
	goto	exit_int
...
;	goto	$-1
	...
	clrf 	TMR1H
  	...
  	btfss	INTCON,T0IF
	goto	$-1
    .assert "(tmr1l == 0x00 && tmr1h == 4) || (tmr1l == 0xff && tmr1h == 3),'*** FAILED p16f1825 capacitance shift'"
	nop
	clrf	CPSCON0
	clrf 	T1CON
	BANKSEL	FVRCON
...
test_ioc:
   ...
   .assert "W == 0x01, '*** FAILED 16f1825 INT test - No int on rising edge'"
        nop
   .assert "iocaf == 0x04, '*** FAILED 16f1825 IOCAF bit 2 not set rising edge'"
	nop
	bcf	INTCON,IOCIF
   ...
   .assert "(intcon & 1) == 0, '*** FAILED 16f1825 INTCON,IOCIF not cleared from IOCAF'"
	nop 
	clrf	inte_cnt
	bsf	INTCON,IOCIE	; Turn interrupts back on
	BANKSEL	PORTA
...
;	Setup - edge interrupt
   ...
   .assert "W == 0x01, '*** FAILED 16f1825 IOC test - No int on falling edge'"
        nop
   .assert "iocaf == 0x04, '*** FAILED 16f1825 IOCAF bit 2 not set falling edge'"
	nop
	BANKSEL	IOCAF
	CLRF	IOCAF
	bsf	INTCON,IOCIE
