...
; <http://www.gnu.org/licenses/lgpl-2.1.html>.
        ...
        ;; simulate a pic 16lF1936.
        ;; Specifically, basic port operation, eerom, interrupts,
	;; a2d, dac, SR latch, capacitor sense, and enhanced instructions


	list    p=16lf1936                ; list directive to define processor
	include <p16lf1936.inc>           ; processor specific variable definitions
...
RESET_VECTOR  CODE    0x000              ; processor reset vector
  ...
  .sim "p16lf1936.xpos = 72"
  .sim "p16lf1936.ypos = 72"
...
;        movwf   status_temp
	...
	.assert "'***FAILED p16lf1936 unexpected interrupt'"
	nop
...
;	;
	...
	clrf	STATUS
	clrf	TRISA
   .assert "trisa == 0x00, '*** FAILED 16lf1936  TRISA not clear '"
	nop
	BANKSEL PORTA
	movlw	0xff
	movwf	PORTA
   .assert "porta == 0xc0, '*** FAILED 16lf1936  analog bits read 0'"
	nop
	movf	PORTA,W
...
;	A2D results here are Vdd dependant
	call test_a2d
	call test_dac
	call test_a2d_ref
 	call write_prog

  .assert  "'*** PASSED 16lf1936 Functionality'"
	nop
	reset
	goto	$
...
test_sr:
    ...
    .assert "(porta&0x30) == 0x20, '*** FAILED p16lf1936 RS Q=0'"
	nop
	banksel PORTB
	bsf 	PORTB,0
   .assert "(porta&0x30) == 0x10 , '*** FAILED p16lf1936 RS Q=1'"
	nop
	banksel	SRCON1
	movlw	1<<SRRPE
	movwf	SRCON1
    .assert "(porta&0x30) == 0x20, '*** FAILED p16lf1936 RS Q=0 SRRPE'"
	nop
	bsf 	SRCON1,SRSPE
    .assert "(porta&0x30) == 0x20 , '*** FAILED p16lf1936 RS Q=0 SRRPE & SRSPE'"
	nop
	clrf 	SRCON1
	bsf 	SRCON0,SRPS
    .assert "(porta&0x30) == 0x10, '*** FAILED p16lf1936 RS Q=1 SRPS'"
	nop
	bsf 	SRCON0,SRPR
    .assert "(porta&0x30) == 0x20 , '*** FAILED p16lf1936 RS Q=0 SRPR'"
	nop
...
;	.command "detach porta5 n4"
	...
	movwf	DACCON0
	movlw	0x02	; Ladder 2/32 * 3.3V = 0.2063 V
	movwf	DACCON1
	BANKSEL	ADCON0
	...
	movwf   ADCON0      	
	; DAC output  (1V - 0.2063) /(3.3-0.2063) = 0.25655
	; 0.x25655 * 1024 = 262 = 0x106
	call	a2dConvert
   .assert "adresh == 0x01 && (adresl==0x06), '*** FAILED 16lf1936 AN12=1V V-=Vref-pin'"
	nop

  	; Test Vdd = 3.3 as Vsource+ = 1.65 DAC (5 bit)
	BANKSEL DACCON0
	movlw	(1<<DACEN)|(1<<DACOE)
	movwf	DACCON0
	...
	movlw	(1<<ADON)|(0xc<<2)	; V+ = AN12
	movwf   ADCON0      	
	; refhi = 3.3 reflo = 1.65 vin= 1.0
	; (1.0 - 1.65) / (3.3 - 1.65) = -0.3939 x 1024 = -403 = 0xFe6c

	call	a2dConvert
   .assert "adresh == 0x02 && (adresl==0x6e), '*** FAILED 16lf1936 V+=AN12=1V V-=AN1=2.5V 2's comp'"
	nop
	BANKSEL DACCON0
	clrf	DACCON0
	return
...
test_pir_pie_bits:
	...
	movlw	0xff
	movwf	PIR1
   .assert "pir1 == 0xcf, '*** FAILED 16lf1936 PIR1 write test'"
	nop
	clrf	PIR1
	movwf	PIR2
   .assert "pir2 == 0xfd, '*** FAILED 16lf1936 PIR2 write test'"
	nop
	clrf	PIR2
	movwf	PIR3
   .assert "pir3 == 0x7a, '*** FAILED 16lf1936 PIR3 write test'"
	nop
	clrf	PIR3

	BANKSEL	PIE1
	movwf	PIE1
   .assert "pie1 == 0xff, '*** FAILED 16lf1936 PIE1 write test'"
	nop
	clrf	PIE1
	movwf	PIE2
   .assert "pie2 == 0xfd, '*** FAILED 16lf1936 PIE2 write test'"
	nop
	clrf	PIE2
	movwf	PIE3
   .assert "pie3 == 0x7a, '*** FAILED 16lf1936 PIE3 write test'"
	nop
	clrf	PIE3
	return
test_dac:
...
;RRR	bsf     ANSELC,0	;Set RC0 to analog
	...
	call	a2dConvert
   .assert "(adresh==0x00) && (adresl==0x00), '*** FAILED 16lf1936 DAC default'"
	nop
	; test DAC enabled Vout = 3.3V * (16/32) = 1.65
	BANKSEL DACCON0
	movlw	0x10
	movwf	DACCON1
	movlw	(1<<DACEN)
	...
	BANKSEL ADCON0
	call	a2dConvert
   .assert "(adresh==0x01) && (adresl==0xff), '*** FAILED 16lf1936 DAC enabled 1/2'"
	nop
  	; Test  FVR = 3.3 as Vsource+ = 1.65
	BANKSEL	FVRCON
	movlw	(1<<FVREN)|(1<<CDAFVR1)|(1<<CDAFVR0)
	movwf	FVRCON
	btfss	FVRCON,FVRRDY
	goto	$-1
	movlw	(1<<DACEN) | (1<<DACPSS1) | (1<<DACOE)
	movwf	DACCON0
	BANKSEL ADCON0
	call	a2dConvert
   .assert "(adresh==0x01) && (adresl==0xff), '*** FAILED 16lf1936 DAC enabled 1/2 FVR '"
	nop
	
  	; Test DACOUT pin with DAC at 1/2 3.3V = 1.65V
	BANKSEL ANSELA
	bcf	ANSELA,2 ;DACOUT digital
	banksel TRISA
	bsf	TRISA,5	;AN4 analog input
	BANKSEL DACCON0
	bcf	DACCON0,DACPSS1
   ...
   .command "n4"
	...
	movwf   ADCON0      	
	call	a2dConvert
   .assert "(adresh==0x01) && (adresl==0xff), '*** FAILED 16lf1936 DAC output 1/2 Vdd '"
	nop
	clrf	ADCON0	; clear A2D registers
	clrf	ADCON1
	BANKSEL DACCON0
	clrf	DACCON0 ;turn off DAC
...
test_a2d
	...
	movwf   ADCON0      	
	call	a2dConvert
   .assert "adresh == 0x4d && (adresl==0x80), '*** FAILED 16lf1936 AN12=1V'"
	nop
  ...
  ; measure FVR (2.048)
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x9e) && (adresl==0xc0), '*** FAILED 16lf1936 ADC FVR'"
	nop
  ...
  ; measure FVR using FVR Reference
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x03) && (adresl==0xff), '*** FAILED 16lf1936 ADC FVR with FVR Ref'"
	nop
...
eefail:
  .assert "'***FAILED 16lf1936 eerom write/read error'"
	nop
...
test_int:
	...
	movlw	0x7f
	movwf	INTCON
   .assert "intcon == 0x7e, '*** FAILED 16lf1936 INT test - INTCON:IOCIF read only'"
	nop
	BANKSEL	IOCBF 
	movlw	0xff
	movwf	IOCBF
 .assert "iocbf == 0xff, '*** FAILED 16lf1936 INT test - IOCBF writable bits'"
	nop
	clrf	IOCBF
	clrf	INTCON
 	...
 	BANKSEL PORTB
        ...
        nop
        movf    inte_cnt,w
   .assert "W == 0x01, '*** FAILED 16lf1936 INT test - No int on rising edge'"
        nop
   .assert "iocbf == 0x04, '*** FAILED 16lf1936 IOCBF bit 2 not set rising edge'"
	nop
        clrf    inte_cnt
	BANKSEL IOCBF
	clrf	IOCBF
	BANKSEL PORTB
        ...
        nop
        movf    inte_cnt,w
   .assert "W == 0x00, '*** FAILED 16lf1936 INT test - Unexpected int on falling edge'"
        nop
...
;	Setup - edge interrupt
	...
	BANKSEL INTCON
        ...
        nop
        movf    inte_cnt,w
   .assert "W == 0x00, '*** FAILED 16lf1936 INT test - Unexpected int on rising edge'"
        nop
        clrf    inte_cnt
	bsf	INTCON,IOCIE
        bcf     PORTB,5          ; make a falling edge
        nop
        movf    inte_cnt,w
   .assert "W == 0x01, '*** FAILED 16lf1936 INT test - No int on falling edge'"
        nop
   .assert "iocbf == 0x04, '*** FAILED 16lf1936 IOCBF bit 2 not set falling edge'"
	nop
...
read_config_data:
   ...
   .assert "eedath == 0x24 && eedatl == 0x60, '*** FAILED 16lf1936 Device ID'"
	nop
  ...
  ; test write which is in EEDATAH and EEDATAL to userID1
	...
	btfss	PIR2,EEIF
        goto	$-1
  .assert "UserID2 == 0x2460, '*** FAILED 16lf1936 write to UserID2'"
	nop
	return
...
; Test use EEPROM to copy program memory contents
write_prog:
	...
	movlw	0x1f
	movwf	FSR1H		; allow check of result
	bsf	FSR1H,7		; bit 7 of FSRxH must be set for program memory
	movwf	EEADRH          ;
	movlw	0xf0
	movwf	FSR1L
	movwf	EEADRL          ;
	...
	movlw	HIGH write_prog		;   Program memory
	movwf	FSR0H           ;
	movlw	LOW write_prog
	movwf	FSR0L           ;
	moviw	FSR0
	movwf	prog_data
	bsf 	EECON1,EEPGD    ;   Point to program memory
...
START_WRITE
    ...
    .assert "prog_data == W, '***FAILED p16f1937 EEPROM program memory copy'"
	nop
