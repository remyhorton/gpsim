...
Start:	
  ...
  .assert "t0con == 0xff, '***FAILED tmr0_16bit T0CON unexpected reset value'"
	nop
...
TMR0H_ShadowRegisterTest:
  .command  "echo *** Testing TMR0H"
	...
	MOVLW	42
	MOVWF	TMR0H
  .assert "tmr0h == 42, '***FAILED tmr0_16bit TMR0H write error'"
	nop

	MOVWF	TMR0L
  .assert "tmr0l == 42, '***FAILED tmr0_16bit TMR0L write error'"
	nop

	MOVF	TMR0L,W
  .assert "W == 42, '***FAILED tmr0_16bit TMR0L read error'"
	nop

	MOVF	TMR0H,W
  .assert "W == 42, '***FAILED tmr0_16bit TMR0H read error'"
	nop

    ; Clear the shadowed TMR0H register and verify that it cleared.
	CLRF	TMR0H
  .assert "tmr0h == 0, '***FAILED tmr0_16bit clrf TMR0H'"
	nop
	...
	MOVF	TMR0L,W
	MOVF	TMR0H,W
  .assert "W == 42, '***FAILED tmr0_16bit TMR0H not refreshed on TMR0L read'"
	NOP
...
TMR0_8BitModeTest:
 ...
 .command  "echo *** Testing 8-bit mode"
  ...
  ; Stop the timer and clear its value
	...
	CLRF	TMR0L
	CLRF	TMR0H
  .assert "(tmr0l == 0) && (tmr0h == 0), '***FAILED tmr0_16bit clear TMR0L and TMR0H with timer stopped'"
	nop
  ...
  ; All 8 combinations of PSA are tested.
	...
	CLRF	psa	; Shadow's the PSA bits + 1 of T0CON 
...
L1_tmr0_8BitModeTest:   ; Beginning of the loop
  ...
  .command  "psa"
    ...
    ;  If PSA == 0, disable prescaler on TMR0:
...
L_psaInitComplete:
  ...
  .assert "tmr0l == ((tmr0Lo+4)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  .assert "tmr0l == ((tmr0Lo+5)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  .assert "tmr0l == ((tmr0Lo+6)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  .assert "tmr0l == ((tmr0Lo+7)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  .assert "tmr0l == ((tmr0Lo+8)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  .assert "tmr0l == ((tmr0Lo+9)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  .assert "tmr0l == ((tmr0Lo+10)>>psa), '***FAILED tmr0_16bit single cycle check'"
	NOP
  ...
  ; 8 23-cycle delays.
	...
	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+33)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP

	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+56)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP

	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+79)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP

	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+102)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP

	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+125)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP

	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+148)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP
	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+171)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP
	MOVLW	23 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+194)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP
  ; 255 and 256 cycle boundary condition

	MOVLW	61 - 11
	RCALL	DelayCycles
  .assert "tmr0l == ((tmr0Lo+255)>>psa ), '***FAILED tmr0_16bit 23-cycle delay'"
	NOP

  .assert "tmr0l == (((tmr0Lo+256)>>psa)&0xff), '***FAILED tmr0_16bit tmr01 8 bit rap'"
	nop

  ; now for some bigger delays.

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+2*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+3*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+4*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+5*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+6*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+7*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  250
  .assert "tmr0l == (((tmr0Lo+256+8*251)>>psa)&0xff), '***FAILED tmr0_16bit 250 cycle delay'"
	nop

	DELAY  8190
  .assert "tmr0l == (((tmr0Lo+256+8*251+8191)>>psa)&0xff), '***FAILED tmr0_16bit 8190 cycle delay'"
	nop
...
;	test that t0 continues to run after sleep woken up by t1
	...
	nop
	nop
  .assert "tmr0l != W, '*** FAILED tmr0_16bit TMR0 test- TMR0 stopped after sleep'"
	nop
...
;
	...
	sleep
	nop
  .assert "TMR0_RollOver == 0, '*** FAILED tmr0_16bit TMR0 test- TMR0 interrupt in sleep'"
	nop

;------------------------------------------------------------
; 16bit-mode tests

 .command  "echo *** Testing 16-bit mode"
	NOP
	CLRF	psa
...
L_psa16bitInitComplete:
  .command  "psa"
	...
	MOVF	TMR0L,W
  .assert "( ((tmr0h<<8)+W) == (1>>psa)), '***FAILED tmr0_16bit 16bit mode small step'"
	NOP

	MOVF	TMR0L,W
  .assert "( ((tmr0h<<8)+W) == (3>>psa)), '***FAILED tmr0_16bit 16bit mode small step'"
	NOP

	MOVF	TMR0L,W
  .assert "( ((tmr0h<<8)+W) == (5>>psa)), '***FAILED tmr0_16bit 16bit mode small step'"
	NOP

	MOVF	TMR0L,W
  .assert "( ((tmr0h<<8)+W) == (7>>psa)), '***FAILED tmr0_16bit 16bit mode small step'"
	NOP

	MOVF	TMR0L,W
  .assert "( ((tmr0h<<8)+W) == (9>>psa)), '***FAILED tmr0_16bit 16bit mode small step'"
	NOP
...
;	nop
	...
	MOVF	TMR0L,W
	MOVWF	tmr0Lo
  .assert "(((tmr0h<<8)+W) == (1035>>psa)),'***FAILED tmr0_16bit 16bit mode value'"
	NOP
...
;	nop
  ...
  .assert "(((tmr0h<<8)+W) == (9228>>psa)), '***FAILED tmr0_16bit 16bit mode value'"
	NOP
;   .command "((tmr0h<<8)+W)"
;	nop
;  .command  "tmr0Lo"
...
;	nop
  ...
  .assert  "'*** PASSED tmr0_16bit TMR0 test'"
	nop
        bra     $
...
failed:
        movlw   1
        movwf   failures
  .assert  "'*** FAILED tmr0_16bit TMR0 test'"
	nop
        bra     done
