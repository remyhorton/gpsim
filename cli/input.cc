...
private:
    LLInput *LLdata{nullptr};
    LLStack *next_stack{nullptr};
...
void LLStack::print()
{
    if (verbose & 4)
    {
        ...
        for (LLStack *s = Stack; s; s = s->next_stack)
        {
            int depth = 0;

            for (LLInput *h = s->LLdata; h; h = h->next_input)
            {
...
int parse_string_only(const char * str)
{
    ...
    Stack = nullptr;
    const int iRet = parse_string(str);
    delete Stack;
    Stack = OldStack;
    return iRet;
}
...
void process_command_file(const char * file_name, bool bCanChangeDirectory)
{
    ...
    const char *dir_path_end = get_dir_delim(file_name);
    ...
    FILE *cmd_file = fopen(file_name, "r");
    ...
    if (cmd_file)
    {
        ...
        while ((s = fgets(str, 256, cmd_file)) != nullptr)
        {
            if (str[0] == 0 ||
                str[0] == '\n' ||
                (str[1] == '\n' && str[0] == '\r'))
    ...
    else
    {
        ...
        if (GETCWD(cw, _MAX_PATH) == nullptr)
        {
            perror("getcwd()");
        }
        else
               ...
               const char * pProcessorType, const char *pProcessorName)
{
    ...
    if (IsFileExtension(pFileName, "asm"))
    {
    ...
    return CSimulationContext::GetContext()->LoadProgram(
        pFileName, pProcessorType, nullptr, pProcessorName);
}
