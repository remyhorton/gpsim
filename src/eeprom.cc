...
void EEPROM_PIR::callback()
{
  ...
  default:
    std::cout << "EEPROM_PIR::callback() bad eeprom state " << eecon2.get_eestate() << '\n';
    bp.halt();
  }
}
...
void EEPROM_EXTND::start_write()
{
  ...
  if ((eecon1.value.get() & EECON1::EEPGD) || rom_size > 256) {
    wr_adr = eeadr.value.get() + (eeadrh.value.get() << 8);
  } else {
    wr_adr = eeadr.value.get();
  }
  ...
  if (eecon1.value.get() & (EECON1::EEPGD | EECON1::CFGS)) {
    // For short writes, the WR bit will be cleared after 1 cycle.
    // In case of a long write, the processor will execute the next
    // instruction and then ignore the subsequent instruction. The
    // processor will then halt internal operations for typically
    // 2 msec in which the write takes place.
    // Both situations are to be handled in the callback after 1 cycle.
...
void EEPROM_EXTND::callback()
{
  ...
  case EECON2::EEWRITE_IN_PROGRESS:
    ...
    case EECON1::EEPGD:	// write program memory
      if (bp.have_pm_write()) {
        bp.clear_pm_write();
        unsigned int wr_base = wr_adr & ~(num_write_latches - 1);

        if (wr_base >= prog_wp) {
          for (int i = 0; i < num_write_latches; i++) {
            if (write_latches[i] != LATCH_MT) {
              cpu->init_program_memory(cpu->map_pm_index2address(wr_base + i), write_latches[i]);
              write_latches[i] = LATCH_MT;
            }
          }
      ...
      // A device that doesn't have a LWLO bit commits the latches to
      // flash memory when the highest numbered latch is filled.
      if ((eecon1.get_valid_bits() & EECON1::LWLO) || ((wr_adr + 1) % num_write_latches == 0)) {
        // Load the latch
      ...
      // Fall through when only loading latches
