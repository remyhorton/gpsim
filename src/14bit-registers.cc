...
//#define DEBUG
#if defined(DEBUG)
#define Dprintf(arg) {printf("0x%06" G_GINT64_MODIFIER "X %s() ", cycles.get(), __FUNCTION__); printf arg; }
#else
#define Dprintf(arg) {}
#endif
//Debug SR module
...
//#define CDEBUG
#if defined(CDEBUG)
#define CDprintf(arg) {printf("0x%06" G_GINT64_MODIFIER "X %s() ", cycles.get(), __FUNCTION__); printf arg; }
#else
#define CDprintf(arg) {}
#endif
...
//
// An assumption is made that any processor with a OSCCAL register has a calibration value
// in the last program memory location. If this location is not a retlw type instruction 
// a call to this function will write such an instruction into the program memory.
//
...
{
    unsigned int cal_val = pCpu->get_program_memory_at_address(pCpu->program_memory_size()-1);
    if ((cal_val & 0xFF00) != 0x3400)
    {
	pCpu->init_program_memory(pCpu->program_memory_size()-1, 0x3400 | (0xFF & new_value));
    }
...
bool OSCCON::set_rc_frequency(bool override)
{
    ...
    if (old_clock_state != clock_state)
    {
        if ((old_clock_state == LFINTOSC) && clock_state != LFINTOSC)
        {
            ...
            get_cycles().set_break(future_cycle, this);
            CDprintf(("OSCCON future_cycle %" G_GINT64_MODIFIER "d now %" PRINTF_GINT64_MODIFIER "d\n", future_cycle, get_cycles().get()));
...
guint64 OSCCON_1::irc_lh_time()
{
    ...
    CDprintf(("OSCCON_1 LH irc time 4ms %" G_GINT64_MODIFIER "d cycles\n", delay));
    return delay;
}
...
{
    guint64 delay = get_cycles().instruction_cps() * 4e-3;
    CDprintf(("OSCCON_1 POR irc time 4ms %" G_GINT64_MODIFIER "d cycles\n", delay));
    return delay;
}
...
void OSCCON_2::set_callback()
{
    ...
    case HFINTOSC:
        oscstat_new &= ~(OSCSTAT::OSTS | OSCSTAT::PLLR | OSCSTAT::T1OSCR);
        settle = get_cycles().get(2e-6); // 2us settle time
        CDprintf(("OSCCON_2 settle %" G_GINT64_MODIFIER "d\n", settle));
        break;
    ...
    if (cpu_pic->get_int_osc() || (value.get() & SCS1))
    {
        ...
        if (old_clock_state != clock_state)
        {
            if ((old_clock_state == LFINTOSC) && clock_state != LFINTOSC)
            {
                ...
                future_cycle = get_cycles().get() + irc_lh_time();
                get_cycles().set_break(future_cycle, this);
                CDprintf(("OSCCON_HS future_cycle %" G_GINT64_MODIFIER "d now %" PRINTF_GINT64_MODIFIER "d\n", future_cycle, get_cycles().get()));
    ...
    if (cpu_pic->get_int_osc())
    {
        ...
        if (old_clock_state != clock_state)
        {
            if ((old_clock_state == LFINTOSC) && clock_state != LFINTOSC)
            {
                ...
                future_cycle = get_cycles().get() + irc_lh_time();
                get_cycles().set_break(future_cycle, this);
                CDprintf(("OSCCON_HS2 future_cycle %" G_GINT64_MODIFIER "d now %" PRINTF_GINT64_MODIFIER "d\n", future_cycle, get_cycles().get()));
...
{
    Dprintf(("now=0x%" G_GINT64_MODIFIER "x\n", get_cycles().get()));
...
void CPSCON0::set_pin(unsigned int _chan, PinModule *_pin)
{
	Dprintf(("CPSCON0::set_pin pin[%d] = %s (%p)\n", _chan, _pin->getPin().name().c_str(), _pin));
	pin[_chan] = _pin;
...
void CPSCON0::set_chan(unsigned int _chan)
{
    ...
    if (!(value.get() & CPSON))	// module not running
    {
	chan = _chan;
	assert(chan <16);
	return;
...
SRCON0::SRCON0(Processor *pCpu, const char *pName, const char *pDesc, SR_LATCH *_sr_latch)
    : sfr_register(pCpu, pName, pDesc),
      m_sr_latch(_sr_latch)
{
}
...
void SRCON0::put(unsigned int new_value)
{
    ...
    {
        m_sr_latch->pulse_set();
    }

    if ((diff & SRPR) && (new_value & SRPR))
    {
        m_sr_latch->pulse_reset();
    }

    if (diff & CLKMASK)
    {
        m_sr_latch->clock_diff(1<<((new_value & CLKMASK) >> CLKSHIFT));
    }

    if (diff & (SRQEN | SRLEN))
    {
	m_sr_latch->srqen = new_value & SRQEN;
	m_sr_latch->srlen = new_value & SRLEN;
        m_sr_latch->Qoutput();
    }

    if (diff & (SRNQEN | SRLEN))
    {
	m_sr_latch->srnqen = new_value & SRNQEN;
	m_sr_latch->srlen = new_value & SRLEN;
        m_sr_latch->NQoutput();
    }
...
SRCON1::SRCON1(Processor *pCpu, const char *pName, const char *pDesc, SR_LATCH *_sr_latch)
    : sfr_register(pCpu, pName, pDesc), m_sr_latch(_sr_latch)
{
}
...
void SRCON1::put(unsigned int new_value)
{
    ...
    {
        m_sr_latch->srrcke = new_value & SRRCKE;
        m_sr_latch->srscke = new_value & SRSCKE;
        if (!(new_value & (SRRCKE | SRSCKE)))  	// all clocks off
        {
            m_sr_latch->clock_disable();  // turn off clock

        }
        else
        {
            m_sr_latch->clock_enable();  // turn on clock
        }
    }

    m_sr_latch->srsc1e = new_value & SRSC1E;
    m_sr_latch->srsc2e = new_value & SRSC2E;
    m_sr_latch->srspe = new_value & SRSPE;
    m_sr_latch->srrc1e = new_value & SRRC1E;
    m_sr_latch->srrc2e = new_value & SRRC2E;
    m_sr_latch->srrpe = new_value & SRRPE;
    m_sr_latch->update();
}
// SRCON0 for P16f61x
SRCON0_V2::SRCON0_V2(Processor *pCpu, const char *pName, const char *pDesc, SR_LATCH *_sr_latch)
    : sfr_register(pCpu, pName, pDesc),
      m_sr_latch(_sr_latch)
{
    m_sr_latch->cxoutput = true; // comparator output goes through SR module
}
...
void SRCON0_V2::put(unsigned int new_value)
{
    ...
    // Map C1SEN to srsc1e
    if (diff & C1SEN)
	m_sr_latch->srsc1e = new_value & C1SEN;

    // Map C2REN to srrc2e
    if (diff & C2REN)
	m_sr_latch->srrc2e = new_value & C2REN;

    // Map SRCLKEN to srscke
    if (diff & SRCLKEN)
    {
	m_sr_latch->srscke = new_value & SRCLKEN;
	if (m_sr_latch->srscke) 
	    m_sr_latch->clock_enable();
	else
	    m_sr_latch->clock_disable();
    }
    // SR0 0, C1OUT to output, 1, SR Q to output
    if (diff & SR0)
    {
	SRprint((stderr, "SR0=%d\n", new_value & SR0));
        m_sr_latch->sr0 = new_value & SR0;
	m_sr_latch->Qoutput();
    }
...
SRCON1_V2::SRCON1_V2(Processor *pCpu, const char *pName, const char *pDesc, SR_LATCH *_sr_latch)
    : sfr_register(pCpu, pName, pDesc), m_sr_latch(_sr_latch)
{
}
...
void SRCON1_V2::put(unsigned int new_value)
{
    ...
    m_sr_latch->clock_diff(4<<gain);
}
    

// SRCON0 for P16f61x
SRCON0_V3::SRCON0_V3(Processor *pCpu, const char *pName, const char *pDesc, SR_LATCH *_sr_latch)
    : sfr_register(pCpu, pName, pDesc),
      m_sr_latch(_sr_latch)
{
    m_sr_latch->cxoutput = true; // comparator output goes through SR module
...
{
public:
    explicit SRinSink(SR_LATCH *_sr_latch)
        : sr_latch(_sr_latch)
    {}

    void setSinkState(char new3State) override
    {
        sr_latch->setState(new3State);
    }
...
{
public:
    SRnSource(PinModule *_pin, SR_LATCH *_sr_latch, int _index) :
        PeripheralSignalSource(_pin), sr_latch(_sr_latch),
        index(_index)
    {}
    void release() override
    {
        sr_latch->releasePin(index);
    }
private:
    SR_LATCH *sr_latch;
    int       index;
...
SR_LATCH::SR_LATCH(Processor *_cpu) :
...
void SR_LATCH::setIOpin(PinModule *pin, int data)
{
	switch (data)
	{
	    case SRNQ_PIN:
...
void SR_LATCH::set_cxoen(int cm, bool cxoe)
{
...
void SR_LATCH::clock_diff(unsigned int _srclk)
{
    srclk = _srclk;
...
void SR_LATCH::setPins(PinModule *sri, PinModule *srq, PinModule *srnq)
{
    if (sri && !SRI_pin)
    {
...
void SR_LATCH::setState(char /* IOin */ )
{
    if (srspe || srrpe)
    {
        update();
...
void SR_LATCH::syncC1out(bool val)
{
    if (syncc1out != val)
    {
        syncc1out = val;
...
void SR_LATCH::syncC2out(bool val)
{
    if (syncc2out != val)
    {
        syncc2out = val;
...
void SR_LATCH::releasePin(int index)
{
    switch (index)
    {
    case SRQ:
