...
void TMR0::set_tmr0_cpu(Processor *new_cpu, PortRegister *reg, unsigned int pin, OPTION_REG *pOption)
{
    cpu = new_cpu;
...
void TMR0::set_tmr0_cpu(Processor *new_cpu, PinModule *_pin, OPTION_REG *pOption)
{
    cpu = new_cpu;
    m_pOptionReg = pOption;
    if (_pin)
