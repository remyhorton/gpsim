...
void CMxCON0_V2::put(unsigned int new_value)
{
    ...
    if (diff & CxOE)
    {
        if (m_cmModule && m_cmModule->sr_latch && m_cmModule->sr_latch->cxoutput)
        {
            m_cmModule->sr_latch->set_cxoen(cm, new_value & CxOE);
	    get();
	    return;
        }
        cm_output = m_cmModule->cmxcon1[cm]->output_pin(cm);
...
void ComparatorModule2::setIOpin(PinModule *pin, int data)
{
	cmxcon0[data]->cm_output = pin;
}
