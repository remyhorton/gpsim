...
class HLVDCON::HLVD_stimulus : public stimulus {
public:
...
HLVDCON::HLVD_stimulus::HLVD_stimulus(HLVDCON *_hlvd, const char *cPname):
  stimulus(cPname, 2.5, 1e12), hlvd(_hlvd)
{
}


HLVDCON::HLVD_stimulus::~HLVD_stimulus()
{
}


void HLVDCON::HLVD_stimulus::set_nodeVoltage(double v)
{
  nodeVoltage = v;
  hlvd->check_hlvd();
}
...
HLVDCON::HLVDCON(Processor *pCpu, const char *pName, const char *pDesc,
               const double *phlvd_volts, double reference)
  : sfr_register(pCpu, pName, pDesc), 
    valid_bits(0xff), write_mask(0x9f), hlvd_volts(phlvd_volts), hlvd_reference(reference)
{
}
...
{
  unsigned int diff = value.get() ^ (new_value & write_mask);
  trace.raw(write_trace.get() | value.get());
...
void HLVDCON::check_hlvd()
{
  ...
  if ((reg & HLVDL_MASK) == HLVDL_MASK) {	// using HLVDIN pin
    ...
    // High voltage trip ?
    if ((reg & VDIRMAG) && (voltage >= hlvd_reference)) {
      IntSrc->Trigger();
    }

    // Low voltage trip ?
    else if (!(reg & VDIRMAG) && (voltage <= hlvd_reference)) {
      IntSrc->Trigger();
    }

  } else {	// Voltage divider on Vdd
    double voltage = hlvd_volts[reg & HLVDL_MASK];
