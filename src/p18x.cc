...
void P18C2x2::create()
{
    ...
    add_sfr_register(&lvdcon,	  0xfd2, RegisterValue(0x05, 0), "lvdcon");
    lvdcon.setIntSrc(new InterruptSource(pir2, PIR2v2::LVDIF));
    lvdcon.set_hlvdin(&(*m_porta)[5]);
    lvdcon.set_valid_bits(0x3f);
    lvdcon.set_write_mask(0x1f);
}
...
P18C2x2::P18C2x2(const char *_name, const char *desc)
    : _16bit_compat_adc(_name, desc),
      lvdcon(this, "lvdcon", "LVD Control", lvd_volts, 1.2)
{
    if (verbose)
    {
...
const double P18C2x2::lvd_volts[15] = {
    (1.8 + 1.91) / 2.0, (2.0 + 2.12) / 2.0, (2.2 + 2.33) / 2.0,
    (2.4 + 2.54) / 2.0, (2.5 + 2.65) / 2.0, (2.7 + 2.86) / 2.0,
    (2.8 + 2.97) / 2.0, (3.0 + 3.18) / 2.0, (3.3 + 3.50) / 2.0,
    (3.5 + 3.71) / 2.0, (3.6 + 3.82) / 2.0, (3.8 + 4.03) / 2.0,
    (4.0 + 4.24) / 2.0, (4.2 + 4.45) / 2.0, (4.5 + 4.77) / 2.0
};
...
const double P18C4x2::lvd_volts[15] = {
    (1.8 + 1.91) / 2.0, (2.0 + 2.12) / 2.0, (2.2 + 2.33) / 2.0,
    (2.4 + 2.54) / 2.0, (2.5 + 2.65) / 2.0, (2.7 + 2.86) / 2.0,
    (2.8 + 2.97) / 2.0, (3.0 + 3.18) / 2.0, (3.3 + 3.50) / 2.0,
    (3.5 + 3.71) / 2.0, (3.6 + 3.82) / 2.0, (3.8 + 4.03) / 2.0,
...
const double P18F1220::lvd_volts[15] = {
    1.8, // reserved 
    2.0, // reserved
    (1.99 + 2.53) / 2.0,
    (2.16 + 2.75) / 2.0, (2.25 + 2.86) / 2.0, (2.43 + 3.10) / 2.0,
    (2.53 + 3.21) / 2.0, (2.70 + 3.43) / 2.0, (2.96 + 3.77) / 2.0,
    (3.14 + 4.00) / 2.0, (3.23 + 4.11) / 2.0, (3.41 + 4.34) / 2.0,
    (3.58 + 4.56) / 2.0, (3.76 + 4.79) / 2.0, (4.04 + 5.15) / 2.0
};
...
void P18F2x21::create(bool has_usb)
{
    ...
    add_sfr_register(&hlvdcon,	  0xfd2, RegisterValue(0x05, 0), "hlvdcon");
    hlvdcon.setIntSrc(new InterruptSource(pir2, PIR2v2::LVDIF));
    hlvdcon.set_hlvdin(&(*m_porta)[5]);
    hlvdcon.set_valid_bits(0xbf); // no bandgap
}
...
P18F2x21::P18F2x21(const char *_name, const char *desc)
    : _16bit_v2_adc(_name, desc), m_trise(nullptr), m_late(nullptr),
      ...
      hlvdcon(this, "hlvdcon", "High/Low-Voltage Detect Register", hlvd_volts, 1.2),
      osctune(this, "osctune", "OSC Tune"),
      comparator(this)
{
    if (verbose)
...
const double P18F2x21::hlvd_volts[15] = {
    2.17, 2.23, 2.36, 2.44, 2.60, 2.79, 2.89, 3.12,
    3.39, 3.55, 3.71, 3.90, 4.11, 4.33, 4.59
};
...
P18F6x20::P18F6x20(const char *_name, const char *desc)
    : _16bit_v2_adc(_name, desc),
      ...
      lvdcon(this, "lvdcon", "LVD Control", lvd_volts, 1.22),
...
const double P18F6x20::lvd_volts[15] = {
    1.8, // reserved
    2.06, 2.27, 2.47, 2.58, 2.78, 2.89, 3.1,
    3.41, 3.61, 3.72, 3.92, 4.13, 4.33, 4.64
};
