...
public:
  ...
  HLVDCON(Processor *pCpu, const char *pName, const char *pDesc, const double *hlvd_volts, double reference);
  ~HLVDCON();
  ...
  inline void set_write_mask(unsigned int wm)
  {
    write_mask = wm;
  }
...
private:
  ...
  const double     *hlvd_volts = nullptr;
  double           hlvd_reference;
