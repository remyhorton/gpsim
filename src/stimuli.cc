...
void Stimulus_Node::attach_stimulus(stimulus *s)
{
    ...
    if (s->snode) {
        ...
        // Detach the stimulus from its current node
        s->snode->detach_stimulus(s);
    }
...
void stimuli_attach(gpsimObject *pNode, gpsimObjectList_t *pPinList)
{
    ...
    if (psn)
    {
        ...
        //std::cout << __FUNCTION__ << " pNode " << pNode->name().c_str() << " is a Stimulus_Node\n";
...
void stimuli_detach(gpsimObjectList_t *pPinList)
{
    if (!pPinList)
        return;
