...
#endif
#if DEBUG & 2
#include "../config.h"          // Needed for PRINTF_GINT64_MODIFIER
...
void CCPCON::put(unsigned int new_value)
{
	...
	config_output(0, true, false);
        ...
        {
 	    RRprint((stderr, "CCPCON::put %s call tmr2_update\n", name().c_str()));
 	    tmr2->tmr2_update();
        }
...
void CCPCON::capture_start(unsigned int mode, unsigned int old_value)
{
    ...
    // Capture doesn't use an output pin. Setting newOut to true breaks the
    // comparator configuration, if that uses the same I/O pin 
    config_output(0, false, true);
    ccprl->stop_compare_mode();
    switch(mode)
    {
...
void TMR2::tmr2_update()
{

    RRprint((stderr, "TMR2::tmr2_update %s=%d running=%d now=%ld use_clk=%d enabled=%d future_cycle=%ld\n", name().c_str(), value.get(), running, get_cycles().get(), use_clk, enabled, future_cycle));

    //std::cout << "TMR2 tmr2_update. cpu cycle " << std::hex << cycles.get() <<'\n';
	...
	if (fc < get_cycles().get()) // TMR2 > PR2 + 1
	{
	    ...
	    {
                std::cout << name() << " TMR2::tmr2_update note: new breakpoint=" << std::hex << fc <<
                          " before old breakpoint " << future_cycle <<
                          " now " << get_cycles().get() << '\n';
	    }
            if (fc != future_cycle)
            {
                // std::cout << name() << " TMR2::tmr2_update new break at cycle "<<hex<<fc<<'\n';
                get_cycles().reassign_break(future_cycle, fc, this);
                future_cycle = fc;
            }
...
void ECCPAS::put_value(unsigned int new_value)
{
    ...
    new_value &= mValidBits;
    Dprintf(("ECCPAS::put_value(0x%x) mValidBits=0x%x diff=0x%x\n", new_value, mValidBits, new_value ^ old_value));
...
// This decodes the CCxPAS<2:0> bits properly, but looks different
bool ECCPAS::shutdown_trigger(int key)
{
    ...
    if ((key & ECCPAS0) && trig_state[0]) //C1
    {
        return true;
    }

    if ((key & ECCPAS1) && trig_state[1]) //C2
    {
        return true;
    }

    if ((key & ECCPAS2) && trig_state[2]) //Int pin
    {
        return true;
    }
...
{
    bool trig = (bool)state ^ c12_invert;
    RRprint((stderr, "ECCPAS::c1_output %s state=%d trig=%d c12_invert=%d\n", name().c_str(), state, trig, c12_invert));
    set_trig_state(0, trig);
}
...
{
    bool trig = (bool)state ^ c12_invert;
    RRprint((stderr, "ECCPAS::c2_output %s state=%d trig=%d c12_invert=%d\n", name().c_str(), state, trig, c12_invert));
    set_trig_state(1, trig);
}
