...
{
    Dprintf(("TXREG callback - time:%" G_GUINT64_FORMAT "x full %d\n",get_cycles().get(), full));
    if (full)
    {
        mUSART->full();
        full = false;
...
void _TXSTA::transmit_a_bit()
{
    if (bit_count)
    {
        Dprintf(("Transmit bit #%x: bit val:%u time:0x%" G_GUINT64_FORMAT "x\n",
...
{
    Dprintf(("TXSTA callback - time:%" G_GUINT64_FORMAT "x\n", get_cycles().get()));
...
void _RCSTA::receive_a_bit(unsigned int bit)
{
    ...
    Dprintf(("%s receive_a_bit state:%u bit:%u time:0x%" G_GUINT64_FORMAT "x\n",
...
void _RCSTA::receive_start_bit()
{
    ...
    // The usart samples just before the middle of the bit three times and
    // produces a sample based on majority averaging.
    //
    // 18.4.3:
    // https://ww1.microchip.com/downloads/en/DeviceDoc/31018a.pdf
    //
    // Sampling is done at 7th, 8th and 9th cycle of 16 cycles.
    // This is all fine but we have to work with CPU cycles here which is
    // already divided by 4 so the resolution is quite bad.
    //
    // As an extreme example it's possible to do 115200 baud with a CPU clock
    // of just 921 kHz on a 18F1320 which is only 2 cycles per bit.
    //
    // So the strategy is take half the bit time and additionally sample 2/16
    // and 1/16 earlier. For large number of cycles divisible by 16 that's
    // exactly 6/16, 7/16, 8/16 (as the document counts from 1). For small number
    // of cycles with careful rounding it degrades to something sensible like
    // all 3 sampling points in the middle.

    if (cpu) {
        unsigned int cycles_per_tick = spbrg->get_cycles_per_tick();
        // First sampling is the 7th cycle of 16. That's 2/16 earlier than the half.
        unsigned int time_to_event = cycles_per_tick / 2 - (cycles_per_tick * 2) / 16;
        // Must be in the future
        if (time_to_event < 1) time_to_event = 1;
...
{
    Dprintf(("RCSTA callback. %s time:0x%" G_GUINT64_FORMAT "x\n", name().c_str(), get_cycles().get()));
    ...
    {
        unsigned int cycles_per_tick, time_to_event;
        ...
        case RCSTA_WAITING_MID1:
            ...
            cycles_per_tick = spbrg->get_cycles_per_tick();
            // Second sampling is the 8th cycle of 16. It's the 7th now
            // so do another 2/16 and retract by 1/16.
            time_to_event = (cycles_per_tick * 2) / 16 - cycles_per_tick / 16;

            sample_state = RCSTA_WAITING_MID2;

            if (cpu && time_to_event > 0) {
                get_cycles().set_break(get_cycles().get() + time_to_event, this);
                break;
            }
            /* fall through if it's immediate */
        ...
        case RCSTA_WAITING_MID2:
            ...
            cycles_per_tick = spbrg->get_cycles_per_tick();
            // Third sampling is the 9th cycle of 16. It's the 8th now
            // so do another 1/16.
            time_to_event = cycles_per_tick / 16;
        ...
        case RCSTA_WAITING_MID3:
            ...
            if(state==RCSTA_RECEIVING)
            {
                if (cpu) {
                    cycles_per_tick = spbrg->get_cycles_per_tick();
                    // Do another round. It's the 9th of 16 now and the next is the
                    // 7th, that's 2/16 less.
                    time_to_event = cycles_per_tick - (cycles_per_tick * 2) / 16;
                    // Must be in the future.
                    if (time_to_event < 1) time_to_event = 1;
...
void _SPBRG::get_next_cycle_break()
{
    ...
    if (cpu)
    {
        ...
        {
            Dprintf(("%s future %" G_GUINT64_FORMAT "d <= now %" G_GUINT64_FORMAT "d\n", name().c_str(), future_cycle, get_cycles().get()));
    ...
    //Dprintf(("SPBRG::callback next break at 0x%" G_GUINT64_FORMAT "x\n",future_cycle));
}
...
void _SPBRG::start()
{
    ...
    Dprintf((" SPBRG::start   last_cycle:0x%" G_GUINT64_FORMAT "x: future_cycle:0x%" G_GUINT64_FORMAT "x\n",last_cycle,future_cycle));
}
...
void _SPBRG::put(unsigned int new_value)
{
    ...
    future_cycle = last_cycle + get_cycles_per_tick();
    skip = future_cycle;
    Dprintf((" SPBRG value=0x%x skip=0x%" G_GUINT64_FORMAT "x last=0x%" G_GUINT64_FORMAT "x cycles/tick=0x%x\n",value.get(), skip, last_cycle, get_cycles_per_tick()));
}
...
void _SPBRG::callback()
{
    if (skip)
    {
        Dprintf((" SPBRG skip=0x%" G_GUINT64_FORMAT "x, cycle=0x%" G_GUINT64_FORMAT "x\n", skip, get_cycles().get()));
    }
    if (! skip  || get_cycles().get() >= skip)
    {
        last_cycle = get_cycles().get();
    ...
    //Dprintf(("SPBRG rollover at cycle:0x%" G_GUINT64_FORMAT "x\n",last_cycle));
