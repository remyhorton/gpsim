/*
   ...
   Copyright (C) 2009, 2013, 2017, 2025 Roy R. Rankin
...
void ADCON1_16F::put_value(unsigned int new_value)
{
    ...
    Dprintf(("ADCON1_16F::put_value %s new_value=0x%x valid_bits=0x%x\n", name().c_str(), new_value, valid_bits));
...
{
    Dprintf(("ADCON1_16F::getVrefLo ADNREF=%d Vreflo_position[%d] = %d\n", (ADNREF & value.get()), cfg_index, Vreflo_position[cfg_index]));
    if (ADNREF & value.get())
    {
...
int ADCON1::getChannelNumber(PinModule *pinModule)
{
	for(unsigned int i = 0; i< m_nAnalogChannels; i++)
	{
	    if (m_AnalogPins[i] == pinModule) return(i);
	}
	return -1;
}
...
{
    Dprintf(("set_FVR_volt %s %.3f %u nchan=%d\n", name().c_str(), in_value, channel, m_nAnalogChannels));
    if (channel < m_nAnalogChannels)
    {
        m_voltageRef[channel] = in_value;
...
double ADCON1::getChannelVoltage(unsigned int channel)
{
    ...
    if (channel < m_nAnalogChannels)
    {
	Dprintf(("ADCON1::getChannelVoltage channel=%d cfg_index=%d test=%x \n", channel, cfg_index, (1 << channel) & m_configuration_bits[cfg_index]));
		...
		Dprintf(("ADCON1::getChannelVoltage %s %.3f\n", pm->getPin().name().c_str(), voltage));
...
void ANSEL_P::setAdcon1(ADCON1 *new_adcon1, PortRegister *port )
{
    adcon1 = new_adcon1;
    if (port)
	m_port = port;
...
void ANSEL_P::put(unsigned int new_value)
{
    ...
    for (i = 0; i < 8; i++)
    {
		...
		if (m_port && adcon1)    
		{
		    ...
		    {
		        channel = adcon1->getChannelNumber(m_port->getIOpins(i));
    Dprintf(("ANSEL_P::put %s channel = %d %s[%d]\n", name().c_str(), channel, m_port->name().c_str(), i));
			if (channel >= 0)
			{
			    a2dChannel[i] = channel;
			    chan = channel;
			}
			else
			{
			    // Analog channels not used by A2D
    			    if (otherChannel[i] < 0)
			    {
				std::string pin_name = "AN";
				pin_name.append(m_port->getIOpins(i)->getPin().name().substr( 4, 4));
                		m_port->getIOpins(i)->AnalogReq(this, true, pin_name.c_str());
    			        otherChannel[i] = i;
			    }
			}
		    }
	    ...
	    {
		    if (otherChannel[i] >= 0) //was set before now clear
		    {
			otherChannel[i] = -1;
                	m_port->getIOpins(i)->AnalogReq(this, false, m_port->getIOpins(i)->getPin().name().c_str());
		    }
	    }
            chan++;
        }
    ...
    {
	unsigned int amask = an_ansel->get_mask();
	Dprintf(("ANSEL_P::put %s %s amask = 0x%x mask=0x%x\n", name().c_str(), an_ansel->name().c_str(), amask, mask));
        mask |= amask;
    }

    Dprintf(("ANSEL_P::put %s mask=0x%x\n", name().c_str(), mask));
...
double FVRCON::compute_FVR_AD(unsigned int fvrcon)
{
    ...
    double Vmax = ((Processor *)cpu)->get_Vdd();
...
void DACCON0::set_dacoutpin(bool output_enabled, int chan, double Vout)
{
    ...
    Dprintf(("DACCON0::set_dacoutpin output_enabled=%d chan=%d Vout=%.3f pin=%p\n", 
			    output_enabled, chan, Vout, output_pin[chan]));
    if (output_pin[chan])
    {
    ...
    if (output_enabled)
    {
	Dprintf(("DACCON0::set_dacoutpin Pin_Active[%d]=%d \n",chan, Pin_Active[chan])); 
	    ...
	    orig_name[chan] = output_pin[chan]->getPin().GUIname();
	    output_pin[chan]->getPin().newGUIname(pin_name.c_str());
	Dprintf(("DACCON0::set_dacoutpin Pin_Active[%d]=%d pin_name %s\n",chan, Pin_Active[chan], pin_name.c_str())); 
...
void  DACCON1::put_value(unsigned int new_value)
{
    unsigned int masked_value = (new_value & bit_mask);
    value.put(masked_value);
    Dprintf(("DACCON1::put_value  daccon0=%p new_value 0x%x bit_mask 0x%x\n", daccon0, new_value, bit_mask));
