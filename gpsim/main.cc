...
static const char *cmf_name     = nullptr;
...
#define POPT_MYEXAMPLES { NULL, '\0', POPT_ARG_INCLUDE_TABLE, myHelpOptions, \
  ...
  gpsim -p p16f877 myprog.hex -s myprog.cmf \n\
                                       <--load hex, MPLAB cmf symbol file\n\
...
struct poptOption optionsTable[] = {
  ...
  },
  {
    ...
    ".cod or .cmf symbol file (-s optional)", nullptr
  },
  {
...
int main(int argc, char *argv[])
{
...
#ifdef EXIT	// looks like dead code RRR 12/23
      case 'e':
  ...
  if (cod_name && strstr(cod_name, ".cmf")) {
    cmf_name = cod_name;
    cod_name = nullptr;
  }
  ...
  try {
    ...
    if (cmf_name && (!hex_name || !processor_name)) {
        std::cerr << "ERROR: for .cmf file, hex file and processor must be specified.\n";
        exit_gpsim(1);
    }
