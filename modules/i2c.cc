...
// I2C_WriteOperation
class I2C_WriteOperation : public Boolean {

public:
  explicit I2C_WriteOperation();
};
    ...
    m_MSBmask(1 << 8)
{
  ...
  mWriteOp = new I2C_WriteOperation();
...
void I2CMaster::new_scl_edge(bool direction)
{
  ...
  if (direction) {
    ...
    switch (m_uState) {
    case eI2CTransferD:
      if (m_bitCount == 1 && m_mState == eI2CTransfer && !writeOp) {
          mRxByte->setFromMaster(m_xfr_data & 0xff);
          mRxSequence->setFromMaster(mRxSequence->getVal() + 1);
      }
      if (readBit()) {
...
I2CMaster::eI2CResult I2CMaster::send8BitData(unsigned int data)
{
  ...
  if (isSCLlow()) {
    ...
    if (m_mState == eI2CIdleBus) {
        setNextMacroState(eI2CAddress);
    } else {
        setNextMacroState(eI2CTransfer);
    }
    m_bitCount = 9;             // There are 8 data bits and 1 ACK bit
    if (m_mState == eI2CAddress || writeOp) {
        // The target device must acknowledge the reception of the address
        // frame, as well as all data frames in a write communication
        // To accomodate this, the controller must release SDA during ACK
        m_xfr_data = data << 1 | 1;
    } else {
        // When reading from a target device, the controller must release SDA
        // while clocking in the data bits and then send an ACK for all but
        // the final frame
        m_xfr_data = 0xff << 1 | (data != 0);
    }
    m_nextBit = (m_xfr_data & m_MSBmask) == m_MSBmask;
...
void I2CMaster::send7BitAddress(unsigned int addr)
{
  ...
  if (eI2CResSuccess == sendStart()) {
    ...
    writeOp = mWriteOp->getVal();
    if (!writeOp) data |= 1;
    mTxByte->setFromMaster(data);
  }
}
...
//########################################
// TxBuffer - initiate data transfer - In case of a write operation this is
// the byte currently transmitting. For a read operation this specifies if
// this is the final frame (no ACK will be sent).
...
//########################################
I2C_WriteOperation::I2C_WriteOperation()
  : Boolean("write_op", true,
        "I2C Write Operation - control weather the\n"
        "next I2C communication is a write operation.")
{       
}
